...
def welcome():
    exit_game = False
    while not exit_game:
        gameWindow.fill((233,0,0))
        text_screen("Welcome TO Snake Game",black,200,250)
        text_screen("Press Space to Play",black,200,210)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                exit_game = True
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    gameWindow.fill((0,233,0))
                    gameloop()
...
def gameloop():
    ...
    food_x = random.randint(45, screen_width-100)
    food_y = random.randint(55, screen_height-100)
    score = 0
    init_velocity = 5
    snake_size = 15
    fps = 30
